...
import type { MetroInventoryRow } from '@/lib/types/campaign-planning';
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const { data: metrosData, error } = await supabase.rpc('get_metro_inventory');

    if (error) throw error;

    const metros = metrosData as MetroInventoryRow[] | null;

    // Format trends (placeholder - will be real data after DMA added)
    const trends = (metros || []).slice(0, 20).map((metro: MetroInventoryRow) => ({
      metro: metro.metro,
    ...
    return NextResponse.json(
      { error: 'Failed to fetch inventory trends', details: errorMessage },
      { status: 500 }
    );
  }
}
