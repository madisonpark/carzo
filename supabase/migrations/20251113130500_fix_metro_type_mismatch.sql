-- Fix type mismatch - COALESCE returns VARCHAR, function expects TEXT
...
BEGIN
  ...
  WITH metro_stats AS (
    SELECT
      COALESCE(v.dma, v.dealer_city || ', ' || v.dealer_state)::TEXT as metro,
  ...
  body_style_breakdown AS (
    ...
    FROM (
      SELECT
        COALESCE(v.dma, v.dealer_city || ', ' || v.dealer_state)::TEXT as metro_name,
        v.body_style,
        COUNT(*) as cnt,
        ROW_NUMBER() OVER (
