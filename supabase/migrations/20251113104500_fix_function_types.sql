-- Fix return type mismatches by casting to TEXT

DROP FUNCTION IF EXISTS get_body_style_inventory();
DROP FUNCTION IF EXISTS get_make_inventory();

-- Function: Get inventory breakdown by body style (TYPE-SAFE)
...
BEGIN
  ...
  metro_breakdown AS (
    ...
    FROM (
      SELECT
        v2.body_style::TEXT,
        (v2.dealer_city || ', ' || v2.dealer_state)::TEXT as metro,
        COUNT(*) as cnt,
...
-- Function: Get inventory breakdown by make (TYPE-SAFE)
...
BEGIN
  ...
  metro_breakdown AS (
    ...
    FROM (
      SELECT
        v3.make::TEXT,
        (v3.dealer_city || ', ' || v3.dealer_state)::TEXT as metro,
        COUNT(*) as cnt,
